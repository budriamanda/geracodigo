...
const SLUG = 'gerador-de-etiquetas'
const EtiquetaGeneratorClient = dynamic(() => import('./EtiquetaGeneratorClient'), {
...
export default async function EtiquetasPage() {
  ...
  const faqs = faqsAll
    .filter((f) => f.entry.pagina === 'etiqueta')
  ...
  const adPrefix = tool.adSlotPrefix || 'etiqueta'
  const sections = (tool.secoesConteudo || []) as unknown as ContentSection[]

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <ToolEngagementTracker toolName="etiqueta_generator" />
      <SchemaMarkup schema={schemas} />
      <div className="flex justify-center mb-6">
        <AdSlot slot={`${adPrefix}-top`} format="horizontal" />
      </div>
      <Breadcrumb current="Gerador de Etiquetas" />
      <div className="mb-8">
